...
const roll = (
	...
	if (i > 0) {
		const p1 = geoProject(coToLngLat(co));
		const p2 = geoProject(coToLngLat(xs[i - 1]));
		...
		if (distance < m.distance) {
			...
			return {
				index: [...index, i],
				distance,
				coordinate: lngLatToCo(geoUnproject({ x, y }))
			};
		}
	}
	...
	return data.features.reduce((m1, feature, i) => {
		...
		if (type === MULTI_POINT) {
			if (properties.type === CIRCLE) {
				...
				const p1 = geoProject(coToLngLat(co1));
				const p2 = geoProject(coToLngLat(co2));
				const p3 = sub(p0, p1);
				...
				return distance < m1.distance
					? {
						index: [i],
						distance,
						coordinate: lngLatToCo(geoUnproject(px))
					}
					: m1;
			}
		}
